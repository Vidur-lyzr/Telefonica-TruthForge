...
- Prefer content-bearing layouts from the catalogue below for the substance; do not pad with dividers.
...
- Every deck MUST carry the full visual language, content permitting: at least one figure-led slide whenever the document contains numbers, and at least one icon/card slide for pillars, takeaways or processes — pick the concrete layouts from the catalogue below. Skip a family only when the document truly has no content for it.${hasCharts ? '\n- On campaign-metrics, set "chartKind" when the data shape is clear: "waterfall" for a bridge of deltas to a total, "gauge" for a single share-of-target percentage, otherwise leave it "auto".' : ""}
- Use each layout's purpose line to decide where it fits. Never use a chart layout when no chart series is listed.`
}

...
- Data: browse the corpus (docs, chunks, axes) that backs answers. Ready (and crash-interrupted "failed") non-PDF extraction jobs have a Marca-admin "Rebuild" action (POST /data/master-decks/jobs/rebuild): re-runs extraction from the stored parts with the current quality rules, refreshes families/thumbnails (new versioned asset keys), auto-repairs approved layouts in place (slide-overlap ≥0.5 keeps the old xl- id and approver; families with no surviving match get their layouts removed), audited as config_change — this is the no-reupload fix path for junk layouts approved under old proposer rules. Data Center → Templates additionally ingests master decks (.pptx, or split multi-part uploads; PDF is images-only fallback): server pipeline parses slides, clusters recurring visual families, proposes layout templates with rendered thumbnails plus an image-harvest queue; brand-clearance users review/approve in a drawer. Approved layouts compile into the live visual-layout registry (`data/extractedLayoutStore.ts` snapshot-persisted; `export/deckPipeline.ts`, `export/deckExtract/`) and become available to Generate visual decks alongside the coded layouts; confirmed harvest images land in the brand image library. Extraction has quality gates (`assessCluster` in `deckExtract/propose.ts`): content-dense slides (>8 texts / >5 images / >14 fills) are skipped (noted in the job summary), slots are capped (4 text / 2 bullets / 3 images / 7 total), decorative fills/lines are filtered after slot selection, and proposals get structural names ("Image + text split — Deck"). A "Live extracted layouts" section in Templates lets the brand admin remove approved extracted layouts (POST /data/master-decks/layouts/remove, Marca-admin gated, audited). The full layout pool (coded + extracted, with server-rendered sample slides via `export/layoutPool.ts`) is browsable in Generate's template picker for visual-deck templates, which also shows cover/body page previews for every corporate template.
- Platform Audit (formerly "Observatory"; nav heading "Audit", item "Platform Audit"): audit panel (`/observatory`) for BOTH lyzr and accenture teams; the overview shows ALL tracked users by default (lyzr activity included — no filter checkbox), tracking every user's logins, page visits and time-on-page (sid in session token, 90s activity-window attribution to the previous page), ask turns (question + exact response + persona + governance status + citations), generations (full draft body + refine turns), exports (every exported file's exact bytes are stored in object storage under `audit-exports/` at export time — `export/auditAssets.ts` — and downloadable from the audit trail via `/observatory/export-assets/:assetId/file`; storage failure never blocks the user's export, the event then carries metadata only), Data Center ingestion (searches, accepted docs, uploads — kind `ingest`) and governance/admin changes (user CRUD, source labels, axes, retagging, rollbacks — kind `config_change`). Server-gated by `requireAuditTeam` (lyzr + accenture, 403 code:"forbidden"); nav item is cosmetic. Store: `data/observatoryStore.ts`, persisted to PostgreSQL (`observatory_events` / `observatory_sessions`) — audit history is shared across autoscale instances and survives redeploys.

...
Densification variant: to add chunks to EXISTING docs collision-free, append via an overlay merged at load time with new chunk ids suffixed so original ids (citations, lineage) are never touched; one doc per batch, merge throws on cross-batch duplicates.
